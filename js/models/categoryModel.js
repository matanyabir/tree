...
const CategoryModel = Backbone.Model.extend(
{
	 ...
	 * @param data: the {items, name, isCollapse} JSON
	*/
	,buildFromJson: function (data)
	{
		const {isCollapse, name} = data;
		...
		this.set({isCollapse, name, items});
		return this;
	}

	/**
	 * recursively get all the items JSON
	 * @return data: the {items, name, isCollapse} JSON
	*/
	,getJson: function ()
	{
